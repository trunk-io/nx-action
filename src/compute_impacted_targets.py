...
if not nx_workspace_path:
    print("No Nx workspace path found. Exiting.")
    sys.exit(1)
...
run_command(f"cat {affected_json_out}")
...
num_impacted_projects=run_command(f"wc -l < '{impacted_projects_out}'")
...
run_command("cat '${impacted_projects_out}'")
